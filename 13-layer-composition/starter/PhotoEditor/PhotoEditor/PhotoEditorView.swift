...
final class PhotoEditorDataModel: ObservableObject {
    ...
    init() {
        self.renderingTool = try! LayerRenderingTool(device: self.device)
    }
    ...
    func layersData() -> [LayerData] {
        return self.renderingTool?.layers.enumerated().compactMap { index, layer in
            ...
            let effectDatas = [blendModeEffectData, transformEffectData]
    ...
    func addLayer(selectedImageUrl: URL?) {
        if let selectedImageUrl {
            do {
                let surface = try ContentLayerSurface(device: device, backgroundImage: selectedImageUrl)
