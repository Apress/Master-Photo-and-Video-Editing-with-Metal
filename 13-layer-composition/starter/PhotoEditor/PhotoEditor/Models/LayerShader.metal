...
fragment float4 textureFragmentShader(VertexOutput in [[stage_in]], texture2d<float> texture [[texture(0)]])
{
    constexpr sampler textureSampler(mag_filter::linear, min_filter::linear);
    float4 colorSample = float4(texture.sample(textureSampler, in.uv));
    return colorSample;
}
