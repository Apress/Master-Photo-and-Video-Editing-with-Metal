...
struct Transform {
    ...
    func makeMatrix(width: Int, height: Int) -> matrix_float4x4 {
        ...
        let scaleMatrix = matrix_float4x4(diagonal: [transformedScale.x, transformedScale.y, 1, 1])
...
final class Layer: ObservableObject {
    ...
    init(layer: ContentLayerSurface) {
        ...
        if imgWidth > imgHeight {
            transform.scale = SIMD2(x: 1.0, y: Float(imgHeight) / Float(imgWidth))
        } else if imgHeight > imgWidth {
            transform.scale = SIMD2(x: Float(imgWidth) / Float(imgHeight), y: 1.0)
        }
    }
}
