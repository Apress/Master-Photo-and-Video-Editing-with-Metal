...
final class VideoEditorDataModel: ObservableObject {
    ...
    func setAsset(_ asset: AVURLAsset) {
        Task.init {
            do {
                let renderingTool = try VideoRenderingTool(device: self.device)
                self.renderingTool = renderingTool
                let src = try await VideoComposition(renderingTool: renderingTool, asset: asset)
                let videoComposition = await src.videoComposition

                let playerItem = AVPlayerItem(asset: asset)
                playerItem.videoComposition = videoComposition.copy() as? AVVideoComposition
