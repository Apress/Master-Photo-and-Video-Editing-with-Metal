...
final class GaussianBlurEffect: EffectPass {
    ...
    var kernel: GaussianBlurKernel = GaussianBlurKernel(radius: 10.0, sizeMultiplier: 3.0)
    ...
    var parameters: [any FilterParamaters] {
        [
            SliderParameters(
                binding: .init(get: { self.kernel.radius }, set: { self.kernel = GaussianBlurKernel(radius: $0) }),
                name: "Radius (sigma):",
                step: 1.0,
                range: 0 ... 100,
                value: 10.0
